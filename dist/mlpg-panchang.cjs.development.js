...
var MlpgMoonOutragConst = function MlpgMoonOutragConst() {
  //---------------------------------------------------------------------------
...
var MlpgPanchangImpl = /*#__PURE__*/function () {
  function MlpgPanchangImpl(mlpgConstant) {
    this.mlpgConstant = mlpgConstant;
    this.mlpgMoonOutragConst = new MlpgMoonOutragConst();
  ...
  var _proto = MlpgPanchangImpl.prototype;
  ...
  _proto.moon = function moon(jd) {
    ...
    for (var i = 0; i < 93; i++) {
      // outrage at length
      var arg = this.mlpgMoonOutragConst.corrMoon[i].mlcor * ml + this.mlpgMoonOutragConst.corrMoon[i].mscor * ms + this.mlpgMoonOutragConst.corrMoon[i].fcor * f + this.mlpgMoonOutragConst.corrMoon[i].dcor * d;
      var sinarg = Math.sin(arg);

      if (this.mlpgMoonOutragConst.corrMoon[i].mscor !== 0) {
        sinarg *= i1corr;
        if (this.mlpgMoonOutragConst.corrMoon[i].mscor === 2 || this.mlpgMoonOutragConst.corrMoon[i].mscor === -2) sinarg *= i1corr;
      }

      if (this.mlpgMoonOutragConst.corrMoon[i].fcor !== 0) sinarg *= i2corr;
      lk += this.mlpgMoonOutragConst.corrMoon[i].lcor * sinarg;
    }

    for (var _i = 0; _i < 27; _i++) {
      // outrage at length additional
      var _arg = this.mlpgMoonOutragConst.corrMoon2[_i].ml * ml + this.mlpgMoonOutragConst.corrMoon2[_i].ms * ms + this.mlpgMoonOutragConst.corrMoon2[_i].f * f + this.mlpgMoonOutragConst.corrMoon2[_i].d * d;

      var _sinarg = Math.sin(_arg);

      lk1 += this.mlpgMoonOutragConst.corrMoon2[_i].l * _sinarg;
  ...
  return MlpgPanchangImpl;
}();

var MlpgCalculateFunc = /*#__PURE__*/function () {
  function MlpgCalculateFunc() {}

  var _proto = MlpgCalculateFunc.prototype;

  _proto.calculate = function calculate(d, mlpgConstant) {
    var mlpgPanchangImpl = new MlpgPanchangImpl(mlpgConstant);
    var Day = {};
    var Tithi = {};
    var Paksha = {};
    var Nakshatra = {};
    ...
    var n_wday,
        ...
        n_naksh = 1,
        n_karana = 0,
        n_yoga; //var mlpgPanchangImpl = new MlpgPanchangImpl();
    ...
    var dayhr = day + hr / 24;
    var jdlt = mlpgPanchangImpl.mdy2julian(mon, dayhr, year); // Day:

    n_wday = mlpgPanchangImpl.weekDay(jdlt);
    Day.ino = n_wday;
    Day.name = mlpgConstant.Day.name[n_wday];
    Day.name_en_UK = mlpgConstant.Day.name_en_UK[n_wday]; // julian day at the begining of the day

    var jd0 = mlpgPanchangImpl.mdy2julian(mon, day, year);
    var jdut = jd0 + (hr - tzone) / 24;
    mlpgPanchangImpl.dt = mlpgPanchangImpl.dTime(jdut);
    var jd = jdut + mlpgPanchangImpl.dt / 24; //ayyanamsa

    mlpgPanchangImpl.ayanamsa = mlpgPanchangImpl.calcayan(jd); // Logitudinal Moon

    mlpgPanchangImpl.Lmoon = mlpgPanchangImpl.moon(jd); // Logitudinal Sun

    mlpgPanchangImpl.Lsun = mlpgPanchangImpl.sun(jd); // yoga:

    var zyoga = this.getZYoga(mlpgPanchangImpl, mlpgPanchangImpl.ayanamsa, jd);
    n_yoga = this.getYoga(zyoga);
    var s_yoga = mlpgPanchangImpl.yoga(jd, zyoga, tzone); // Nakstra

    n_naksh = this.getNakshatra(mlpgPanchangImpl, mlpgPanchangImpl.Lmoon, mlpgPanchangImpl.ayanamsa);
    var s_naksh = mlpgPanchangImpl.nakshatra(jd, n_naksh, tzone); // tithi

    n_tithi = this.getTithi(mlpgPanchangImpl.Lmoon, mlpgPanchangImpl.Lsun);
    var s_tithi = mlpgPanchangImpl.tithi(jd, n_tithi, tzone, 12); // paksha

    var n_paksha = this.getPaksha(n_tithi + 1); // Karana

    var KaranaArray = this.getKarana(mlpgPanchangImpl.Lmoon, mlpgPanchangImpl.Lsun);
    n_karana = KaranaArray[0];
    var nk = KaranaArray[1];
    var s_karana = mlpgPanchangImpl.tithi(jd, nk, tzone, 6);
    var z = this.getRaasi(mlpgPanchangImpl, mlpgPanchangImpl.Lmoon, mlpgPanchangImpl.ayanamsa);
    Ayanamsa.name = mlpgPanchangImpl.lon2dms(mlpgPanchangImpl.ayanamsa);
    Raasi.name = mlpgConstant.Raasi.name[z];
    Raasi.ino = z;
    Raasi.name_en_UK = mlpgConstant.Raasi.name_en_UK[z];
    Guna.ino = this.getRaasiGuna(Raasi.ino);
    Guna.name_en_IN = mlpgConstant.Guna.name_en_IN[Guna.ino];
    Guna.name_en_UK = mlpgConstant.Guna.name_en_UK[Guna.ino];
    Guna.name = mlpgConstant.Guna.name[Guna.ino];
    Nakshatra.name = mlpgConstant.Nakshatra.name[n_naksh];
    Nakshatra.name_en_IN = mlpgConstant.Nakshatra.name_en_IN[n_naksh];
    ...
    Trinity.name_en_IN = mlpgConstant.Trinity.name_en_IN[Trinity.ino];
    Trinity.name = mlpgConstant.Trinity.name[Trinity.ino];
    Gana.ino = this.getGanaViaNakshatra(Nakshatra.ino);
    Gana.name_en_IN = mlpgConstant.Gana.name_en_IN[Gana.ino];
    Gana.name_en_UK = mlpgConstant.Gana.name_en_UK[Gana.ino];
    Gana.name = mlpgConstant.Gana.name[Gana.ino];
    Karna.name = mlpgConstant.Karna.name[n_karana];
    Karna.name_en_IN = mlpgConstant.Karna.name_en_IN[n_karana];
    Karna.ino = n_karana;
    Karna.start = s_karana.start;
    Karna.end = s_karana.end;
    Yoga.name = mlpgConstant.Yoga.name[n_yoga];
    Yoga.name_en_IN = mlpgConstant.Yoga.name_en_IN[n_yoga];
    Yoga.ino = n_yoga;
    Yoga.start = s_yoga.start;
    Yoga.end = s_yoga.end;
    Tithi.name = mlpgConstant.Tithi.name[n_tithi];
    Tithi.name_en_IN = mlpgConstant.Tithi.name_en_IN[n_tithi];
    Tithi.ino = n_tithi;
    Tithi.start = s_tithi.start;
    Tithi.end = s_tithi.end;
    Paksha.ino = n_paksha;
    Paksha.name = mlpgConstant.Paksha.name[n_paksha];
    Paksha.name_en_IN = mlpgConstant.Paksha.name_en_IN[n_paksha];
    Paksha.name_en_UK = mlpgConstant.Paksha.name_en_UK[n_paksha];
    Julian.date = jd;
  ...
  _proto.getNakshatra = function getNakshatra(mlpgPanchangImpl, Lmoon, ayanamsa) {
    var Lmoon0 = mlpgPanchangImpl.fix360(Lmoon + ayanamsa);
    return Math.floor(Lmoon0 * 6 / 80);
  };

  _proto.getZYoga = function getZYoga(mlpgPanchangImpl, ayanamsa, jd) {
    mlpgPanchangImpl.moon(jd);
    mlpgPanchangImpl.sun(jd);
    var dmoonYoga = mlpgPanchangImpl.LmoonYoga + ayanamsa - 491143.07698973856;
    var dsunYoga = mlpgPanchangImpl.LsunYoga + ayanamsa - 36976.91240579201;
  ...
  _proto.getRaasi = function getRaasi(mlpgPanchangImpl, Lmoon, ayanamsa) {
    return Math.floor(Math.abs(mlpgPanchangImpl.fix360(Lmoon + ayanamsa)) / 30);
  };
  ...
  return MlpgCalculateFunc;
}();

var MlpgLocalConstant = function MlpgLocalConstant() {
  this.Ritu = {
    name: ['വസന്തം', 'ഗ്രീഷ്മം', 'വര്‍ഷാ', 'ശരത്', 'ഹേമന്തം', 'ശീതം'],
    name_en_UK: ['Spring', 'Summer', 'Monsoon', 'Autumn', 'Pre-Winter', 'Winter']
  };
  this.Masa = {
    name: ['വൈശാഖം', 'ജ്യേഷ്ഠം', 'ആഷാഢം', 'ശ്രാവണം', 'ഭാദ്രപദം', 'ആശ്വിനം', 'കാർത്തികം', 'മാർഗശിരം', 'പൗഷം', 'മാഘം', 'ഫാൽഗുനം', 'ചൈത്രം'],
    name_en_IN: ['Baisakha', 'Jyestha', 'Asadha', 'Srabana', 'Bhadraba', 'Aswina', 'Karttika', 'Margasira', 'Pausa', 'Magha', 'Phalguna', 'Chaitra']
  };
  this.Raasi = {
    name: ['മേടം', 'വൃഷഭം', 'മിഥുനം', 'കർക്കിടകം', 'സിംഹം', 'കന്നി', 'തുലാം', 'വൃശ്ചികം', 'ധനു', 'മകരം', 'കുംഭം', 'മീനം'],
    name_en_UK: ['Aries', 'Taurus', 'Gemini', 'Cancer', 'Leo', 'Virgo', 'Libra', 'Scorpio', 'Sagittarius', 'Capricorn', 'Aquarius', 'Pisces']
  };
  this.Day = {
    name: ['ഞായറാഴ്‌ച', 'തിങ്കളാഴ്‌ച', 'ചൊവ്വാഴ്‌ച', 'ബുധനാഴ്‌ച', 'വ്യാഴാഴ്‌ച', 'വെള്ളിയാഴ്‌ച', 'ശനിയാഴ്‌ച'],
    name_en_UK: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']
  };
  this.Nakshatra = {
    name: ['അശ്വതി', 'ഭരണി', 'കാർത്തിക', 'രോഹിണി', 'മൃഗശിര', 'തിരുവാതിര', 'പുനർതം', 'പൂയം', 'ആയില്യം', 'മകം', 'പൂരം', 'ഉത്രം', 'അത്തം', 'ചിത്തിര', 'ചോതി', 'വിശാഖം', 'അനിഴം', 'ജ്യേഷ്ഠ', 'മൂലം', 'പൂരാടം', 'ഉത്രാടം', 'തിരുവോണം', 'അവിട്ടം', 'ചതയം', 'പൂരുരുട്ടാതി', 'ഉത്രറ്റാതി', 'രേവതി'],
  ...
  this.Paksha = {
    ...
    name: ['ശുക്ലപക്ഷം', 'കൃഷ്ണപക്ഷം']
  };
  this.Tithi = {
    name: ['പ്രതിപത്യം', 'ദ്വിതീയ', 'തൃതീയ', 'ചതുര്‍ത്ഥി', 'പഞ്ചമി', 'ഷഷ്ഠി', 'സപ്തമി', 'അഷ്ടമി', 'നവമി', 'ദശമി', 'ഏകാദശി', 'ദ്വാദശി', 'ത്രയോദശി', 'ചതുര്ദശി', 'പൗര്‍ണമി', 'പ്രതിപത്യം', 'ദ്വിതീയ', 'തൃതീയ', 'ചതുര്‍ത്ഥി', 'പഞ്ചമി', 'ഷഷ്ഠി', 'സപ്തമി', 'അഷ്ടമി', 'നവമി', 'ദശമി', 'ഏകാദശി', 'ദ്വാദശി', 'ത്രയോദശി', 'ചതുര്ദശി', 'അമാവാസ്യ'],
  ...
  this.Karna = {
    ...
    name: ['ബവ', 'ബാലവ', 'കൗളവ', 'തൈതില', 'ഗര', 'വാണിജ്യ', 'വിഷ്ടി', 'ശകുനി', 'ചതുർപ്പദ', 'നാഗ', 'കിംസ്തുഘ്ന']
  };
  this.Yoga = {
    ...
    name: ['വിഷ്‌കുംബം', 'പ്രീതി', 'ആയുഷ്മാന്‍', 'സൗഭാഗ്യം', 'ശോഭന', 'അതിര്‍ഘണ്ടം', 'സുകര്‍മ്മാ', 'ധൃതി', 'ശൂലം', 'ഘണ്ടം', 'വൃദ്ധി', 'ധ്രുവം', 'ബ്യാഘാത്', 'ഹര്‍ഷണം', 'വജ്രം', 'അശുക്', 'വ്യതിപാതം', 'വരിയാന്‍', 'പരിഘം', 'ശിവം', 'സിദ്ധം', 'സാധ്യം', 'ശുഭം', 'ശുക്ലം', 'ബ്രഹ്മ', 'ഇന്ദ്ര', 'വൈധൃതി']
  };
  this.Trinity = {
    name_en_IN: ['Brahma', 'Vishnu', 'Shiva'],
    name: ['ബ്രഹ്മാ', 'വിഷ്ണു', 'ശിവ']
  };
  this.Guna = {
    name_en_IN: ['Rajas', 'Tamas', 'Satva'],
    name_en_UK: ['passion, activity, movement', 'ignorance, inertia, laziness', 'goodness, calmness, harmonious'],
    name: ['രാജസികം', 'തമസികം', 'സാത്വികം']
  };
  this.Gana = {
    name_en_IN: ['Devata', 'Manushya', 'Rakshasa'],
    name_en_UK: ['God', 'Human', 'Demon'],
    name: ['ദേവതാ', 'മനുഷ്യൻ', 'രാക്ഷസൻ']
  };
};


var MlpgPkgConstants = function MlpgPkgConstants() {
  this.GanaMatched = [[6, 6, 0], [5, 6, 0], [1, 0, 6]];
};

var MlpgSunMoonTimer = /*#__PURE__*/function () {
  function MlpgSunMoonTimer() {
  ...
  var _proto = MlpgSunMoonTimer.prototype;
  ...
  return MlpgSunMoonTimer;
}();

var MlpgCalendar = /*#__PURE__*/function () {
  function MlpgCalendar() {}

  var _proto = MlpgCalendar.prototype;

  _proto.calendar = function calendar(mlpgConstant, dt, lat, lng, height) {
    var Tithi = {};
    var Nakshatra = {};
    var Yoga = {};
    var Karna = {};
    ...
    var Trinity = {};
    var mlpgPanchangImpl = new MlpgPanchangImpl(mlpgConstant);
    var mlpgCalculateFunc = new MlpgCalculateFunc();
    var mlpgSunMoonTimer = new MlpgSunMoonTimer();
    var sunRise = mlpgSunMoonTimer.getSunRiseJd(dt, lat, lng, height);
    var nn_tithi = this.getCalendarTithi(sunRise, mlpgPanchangImpl);
    var nn_paksha = mlpgCalculateFunc.getPaksha(nn_tithi);
    var ayanamsaAtRise = mlpgPanchangImpl.calcayan(sunRise);
    var nn_naksh = this.getCalendarNakshatra(mlpgCalculateFunc, mlpgPanchangImpl, ayanamsaAtRise, sunRise);
    var nn_yoga = this.getCalendarYoga(mlpgCalculateFunc, mlpgPanchangImpl, sunRise, ayanamsaAtRise);
    var nn_karana = mlpgCalculateFunc.getKarana(mlpgPanchangImpl.moon(sunRise), mlpgPanchangImpl.sun(sunRise))[0];
    var nn_raasi = this.getCalendarRaasi(mlpgPanchangImpl, mlpgPanchangImpl.sun(sunRise), ayanamsaAtRise);
    var masa = this.getMasa(mlpgPanchangImpl, nn_tithi, sunRise);
    var ritu = this.getRitu(masa.n_maasa);
    Raasi.ino = nn_raasi - 1;
    Raasi.name_en_UK = mlpgConstant.Raasi.name_en_UK[nn_raasi - 1];
    Raasi.name = mlpgConstant.Raasi.name[nn_raasi - 1];
    Guna.ino = mlpgCalculateFunc.getRaasiGuna(Raasi.ino);
    Guna.name_en_IN = mlpgConstant.Guna.name_en_IN[Guna.ino];
    Guna.name_en_UK = mlpgConstant.Guna.name_en_UK[Guna.ino];
    Guna.name = mlpgConstant.Guna.name[Guna.ino];
    Tithi.name = mlpgConstant.Tithi.name[nn_tithi - 1];
    Tithi.name_en_IN = mlpgConstant.Tithi.name_en_IN[nn_tithi - 1];
    Tithi.ino = nn_tithi - 1;
    Paksha.ino = nn_paksha;
    Paksha.name = mlpgConstant.Paksha.name[nn_paksha];
    Paksha.name_en_IN = mlpgConstant.Paksha.name_en_IN[nn_paksha];
    Paksha.name_en_UK = mlpgConstant.Paksha.name_en_UK[nn_paksha];
    Nakshatra.name = mlpgConstant.Nakshatra.name[nn_naksh];
    Nakshatra.name_en_IN = mlpgConstant.Nakshatra.name_en_IN[nn_naksh];
    Nakshatra.ino = nn_naksh;
    Trinity.ino = mlpgCalculateFunc.getTrinityByNakshatra(Nakshatra.ino);
    Trinity.name_en_IN = mlpgConstant.Trinity.name_en_IN[Trinity.ino];
    Trinity.name = mlpgConstant.Trinity.name[Trinity.ino];
    Gana.ino = mlpgCalculateFunc.getGanaViaNakshatra(Nakshatra.ino);
    Gana.name_en_IN = mlpgConstant.Gana.name_en_IN[Gana.ino];
    Gana.name_en_UK = mlpgConstant.Gana.name_en_UK[Gana.ino];
    Gana.name = mlpgConstant.Gana.name[Gana.ino];
    Yoga.name = mlpgConstant.Yoga.name[nn_yoga];
    Yoga.name_en_IN = mlpgConstant.Yoga.name_en_IN[nn_yoga];
    Yoga.ino = nn_yoga;
    Karna.name = mlpgConstant.Karna.name[nn_karana];
    Karna.name_en_IN = mlpgConstant.Karna.name_en_IN[nn_karana];
    Karna.ino = nn_karana;
    Masa.ino = nn_raasi - 1;
    Masa.name = mlpgConstant.Masa.name[nn_raasi - 1];
    Masa.name_en_IN = mlpgConstant.Masa.name_en_IN[nn_raasi - 1];
    MoonMasa.ino = masa.n_maasa - 1;
    MoonMasa.isLeapMonth = masa.is_leap_month;
    MoonMasa.name = mlpgConstant.Masa.name[masa.n_maasa - 1];
    MoonMasa.name_en_IN = mlpgConstant.Masa.name_en_IN[masa.n_maasa - 1];
    Ritu.ino = ritu;
    Ritu.name = mlpgConstant.Ritu.name[ritu];
    Ritu.name_en_UK = mlpgConstant.Ritu.name_en_UK[ritu];
    return {
      Tithi: Tithi,
      Paksha: Paksha,
      Nakshatra: Nakshatra,
  ...
  _proto.getCalendarTithi = function getCalendarTithi(sunRise, mlpgPanchangImpl) {
    var moonPhaseToday = mlpgPanchangImpl.lunarPhase(sunRise);
    var today = Math.ceil(moonPhaseToday / 12);
    var tithi = today; //check for skipped tithi

    var moonPhaseTommorow = mlpgPanchangImpl.lunarPhase(sunRise + 1);
  ...
  _proto.getCalendarNakshatra = function getCalendarNakshatra(mlpgCalculateFunc, mlpgPanchangImpl, ayanamsa, sunRise) {
    var nak_today = mlpgCalculateFunc.getNakshatra(mlpgPanchangImpl, mlpgPanchangImpl.moon(sunRise), ayanamsa);
    var nak_tmrw = mlpgCalculateFunc.getNakshatra(mlpgPanchangImpl, mlpgPanchangImpl.moon(sunRise + 1), mlpgPanchangImpl.calcayan(sunRise + 1));
    var n_nak = nak_today;
  ...
  _proto.getCalendarYoga = function getCalendarYoga(mlpgCalculateFunc, mlpgPanchangImpl, sunRise, ayanamsa) {
    var todayYoga = mlpgCalculateFunc.getYoga(mlpgCalculateFunc.getZYoga(mlpgPanchangImpl, ayanamsa, sunRise));
    var tmorowYoga = mlpgCalculateFunc.getYoga(mlpgCalculateFunc.getZYoga(mlpgPanchangImpl, mlpgPanchangImpl.calcayan(sunRise + 1), sunRise + 1));
  ...
  _proto.getCalendarRaasi = function getCalendarRaasi(mlpgPanchangImpl, Lsun, ayanamsa) {
    var solar_nirayana = mlpgPanchangImpl.fix360(Lsun + ayanamsa);
    return Math.ceil(solar_nirayana / 30);
  };

  _proto.getMasa = function getMasa(mlpgPanchangImpl, tithi, sunrise) {
    var lastNewMoon = sunrise - (tithi - 1);
    var nextNewMoon = sunrise + (29 - (tithi - 1));
    var currentSolarMonth = this.getCalendarRaasi(mlpgPanchangImpl, mlpgPanchangImpl.sun(lastNewMoon), mlpgPanchangImpl.calcayan(lastNewMoon));
    var nextSolarMonth = this.getCalendarRaasi(mlpgPanchangImpl, mlpgPanchangImpl.sun(nextNewMoon), mlpgPanchangImpl.calcayan(nextNewMoon));
...
var MlpgPanchang = /*#__PURE__*/function () {
  function MlpgPanchang() {
    this.mlpgLocalConstant = new MlpgLocalConstant();
    this.mlpgPkgConstants = new MlpgPkgConstants();
    this.mlpgCalculateFunc = new MlpgCalculateFunc();
    this.mlpgSunMoonTimer = new MlpgSunMoonTimer();
    this.mlpgCalendar = new MlpgCalendar();
  }

  var _proto = MlpgPanchang.prototype;

  _proto.calculate = function calculate(dt) {
    return this.mlpgCalculateFunc.calculate(dt, this.mlpgLocalConstant);
  };

  _proto.calendar = function calendar(dt, lat, lng, height) {
    return this.mlpgCalendar.calendar(this.mlpgLocalConstant, dt, lat, lng, height);
  };

  _proto.sunTimer = function sunTimer(date, lat, lng, height) {
    height = height || 0;
    return this.mlpgSunMoonTimer.sunTimer(date, lat, lng, height);
  };

  _proto.getMlpgConstant = function getMlpgConstant(catgory, name) {
    return this.mlpgLocalConstant[catgory][name];
  };

  _proto.setMlpgConstant = function setMlpgConstant(catgory, name, mlpglist) {
    this.mlpgLocalConstant[catgory][name] = mlpglist;
  };

  _proto.getGanaMatched = function getGanaMatched(brideIno, groomIno) {
    return this.mlpgPkgConstants.GanaMatched[brideIno][groomIno];
  };

  return MlpgPanchang;
}();

exports.MlpgPanchang = MlpgPanchang;
//# sourceMappingURL=mlpg-panchang.cjs.development.js.map
